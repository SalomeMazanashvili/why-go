...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!payload.destination_id || !payload.label_en) {
      return NextResponse.json({ error: 'destination_id and label_en are required' }, { status: 400 })
    }
    const s = getAdminSupabase()
    const { data, error } = await s.from('pickup_points').insert(payload).select('id').single()
