...
export default function ServiceCategoryForm({ initial, mode }: Props) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const url = mode === 'create' ? '/api/admin/service-categories' : `/api/admin/service-categories/${item.id}`
  ...
  const remove = async () => {
    if (!item.id) return
    if (!confirm(`Delete "${item.name_ka || item.name_en || 'this category'}"? This cannot be undone.`)) return
    setDeleting(true)
    try {
      const res = await fetch(`/api/admin/service-categories/${item.id}`, { method: 'DELETE' })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
  ...
  return (
    <form onSubmit={save} className="space-y-8">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Link href="/admin/service-categories" className="admin-btn admin-btn-ghost">Cancel</Link>
        {mode === 'edit' && (
